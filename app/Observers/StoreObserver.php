        ...
        if ($visibilityChanged || $statusChanged) {
            ...
            if ($sellerUser) {
                $sellerUser->notify(new StoreStatusUpdatedNotification(
                    $store,
                    $visibilityChanged ? (string) $store->getOriginal('visibility_status') : null,
                    $visibilityChanged ? (string) $store->visibility_status : null,
                    $statusChanged ? (string) $store->getOriginal('status') : null,
                    $statusChanged ? (string) $store->status : null,
                ));
            }
        }
    }
