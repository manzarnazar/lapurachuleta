...
class SellerSettlementSettled extends Notification implements ShouldQueue
{
        ...
        try {
            ...
            if ($isAdmin) {
                $mail->line('A seller settlement has been marked as settled.')
                    ...
                    ->line('Amount: ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? ''))
    ...
    public function toFirebase($notifiable): array
    {
        ...
        return [
            ...
            'body' => $isAdmin
                ? ($sellerName . ' | ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? ''))
                : ('Your wallet credited by ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . '.'),
            'image' => null,
            'data' => [
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => $isAdmin ? 'Settlement Settled' : 'Wallet Credited',
            'message' => $isAdmin
                ? ('Settlement settled for ' . $sellerName . ' amount ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . '.')
                : ('Your wallet has been credited with ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . '.'),
            'type' => NotificationTypeEnum::SETTLEMENT_PROCESS(),
