...
class SellerSettlementCreated extends Notification implements ShouldQueue
{
    ...
    public function toFirebase($notifiable): array
    {
        ...
        return [
            ...
            'body' => $isAdmin
                ...
                : ('Amount ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . ' added.'),
            'image' => null,
            'data' => [
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => $isAdmin ? 'New Settlement Entry' : 'New Earning Added',
            'message' => $isAdmin
                ? ('New settlement for ' . $sellerName . ' amount ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . '.')
                : ('Earning amount ' . number_format((float)$s->amount, 2) . ' ' . ($s->currency_code ?? '') . ' added to your settlement.'),
            'type' => NotificationTypeEnum::SETTLEMENT_CREATE(),
