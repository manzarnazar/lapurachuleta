...
class WalletTransactionOccurred extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): ?MailMessage
    {
        try {
            ...
            return (new MailMessage)
                ...
                ->line('Amount: ' . number_format((float)$t->amount, 2) . ' ' . $t->currency_code)
    ...
    public function toFirebase($notifiable): array
    {
        ...
        return [
            ...
            'body' => 'Amount ' . number_format((float)$t->amount, 2) . ' ' . $t->currency_code . ' | ' . ucfirst((string)$t->status),
            'image' => null,
            'data' => [
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => 'Wallet ' . ucfirst((string)$t->transaction_type),
            'message' => 'A wallet transaction has occurred. Amount: ' . number_format((float)$t->amount, 2) . ' ' . $t->currency_code,
            'type' => NotificationTypeEnum::WALLET_TRANSACTION(),
            'sent_to' => 'seller',
