...
class SellerWithdrawalRequested extends Notification implements ShouldQueue
{
        ...
        try {
            ...
            if ($isAdmin) {
                $mail->line('A seller has submitted a withdrawal request.')
                    ...
                    ->line('Amount: ' . number_format((float)$req->amount, 2) . ' ')
    ...
    public function toFirebase($notifiable): array
    {
        ...
        return [
            ...
            'body' => $isAdmin
                ? ($sellerName . ' requested ' . number_format((float)$req->amount, 2) . '.')
                : ('Amount ' . number_format((float)$req->amount, 2) . ' has been requested.'),
            'image' => null,
            'data' => [
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            'title' => $isAdmin ? 'New Withdrawal Request' : 'Withdrawal Request Submitted',
            'message' => $isAdmin
                ? ('Withdrawal request from ' . $sellerName . ' for ' . number_format((float)$req->amount, 2) . '.')
                : ('A withdrawal request of amount ' . number_format((float)$req->amount, 2) . ' has been submitted.'),
            'type' => NotificationTypeEnum::WITHDRAWAL_REQUEST(),
